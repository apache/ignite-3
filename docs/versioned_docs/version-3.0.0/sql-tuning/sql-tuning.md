...
sidebar_label: SQL Performance Tuning
---
...
Apache Ignite supports the [`EXPLAIN PLAN FOR`](../sql-reference/explain-statement.md) statement that can be used to read the execution plan of a query.
...
Apache Ignite supports the [`EXPLAIN MAPPING FOR`](../sql-reference/explain-statement.md) statement that can be used to track how the query is split and what nodes the subqueries are executed on.
...
- All required [DDL operations](../sql-reference/ddl.md);
- Assigning [access permissions](../administrators-guide/security/authentication.md);
