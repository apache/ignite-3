...
In most cases, write throttling is caused by a slow drive, or a high update rate, and should not be a part of normal node operation. You can track write throttling by using the `storage.aipersist` metrics in the [Available Metrics](../../metrics/metrics-list.md) reference.

## Storage Configuration

In Apache Ignite 3, all storage configuration is consolidated under `ignite.storage` [node configuration](../../config/node-config.md#storage-configuration). For more information on how storage is configured, see [Storage Profiles and Engines](../index.md) documentation.
