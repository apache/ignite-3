...
A storage profile is the Apache Ignite node entity that defines the configuration parameters for a Storage Engine. A [Distribution Zone](../../sql-reference/distribution-zones.md) must be configured to use a set of Storage Profiles declared in the node configuration. A table can only have a single primary storage profile defined.
...
After defining storage profiles and [distribution zones](../../sql-reference/distribution-zones.md), you can create tables using SQL or [from code](../../developers-guide/java-to-tables.md). Both zone and storage profile cannot be changed after table creation.
