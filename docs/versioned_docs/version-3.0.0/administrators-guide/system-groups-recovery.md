...
* [Cluster Management Group (CMG)](lifecycle.md#cluster-management-group)
* [Metastorage Group (MG)](lifecycle.md#cluster-metastorage-group)
...
:::note
Disaster recovery for [data partitions](disaster-recovery.md) is described in a separate page.
:::

You see that the majority has been lost in cluster logs in the console or in the [rotated log files](https://en.wikipedia.org/wiki/Log_rotation). When a RAFT group becomes unavailable, the logs would show something like
...
2. If the above fails, forcefully assign a new majority using the following [CLI command](../ignite-cli-tool.md#disaster-recovery-commands) (manually or via REST): `recovery cluster reset --url=<node-url> --cluster-management-group=<new-cmg-nodes>`.
...
3. If some nodes were down or were unavailable due to a network partition (and hence did not participate in the repair):
   ...
   2. Migrate these nodes to the repaired cluster using the following [CLI command](../ignite-cli-tool.md#disaster-recovery-commands) (manually or via REST): `recovery cluster migrate --old-cluster-url=<url-of-old-cluster-node> --new-cluster-url=<url-of-new-cluster-node>`.
...
2. If the above fails:
   ...
   2. Forcefully assign a new majority using the following [CLI command](../ignite-cli-tool.md#disaster-recovery-commands) (manually or via REST): `recovery cluster reset --url=<existing-node-url> [--cluster-management-group=<new-cmg-nodes>] --metastorage-replication-factor=N`.
