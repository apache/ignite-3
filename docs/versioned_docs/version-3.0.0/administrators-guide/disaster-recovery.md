...
sidebar_label: Disaster Recovery for Data Partitions
---
...
:::note
Disaster recovery for system groups - [Cluster Management Group](system-groups-recovery.md#cluster-management-group) and [Metastorage Group](system-groups-recovery.md#metastorage-group) - is described in a separate page.
:::
...
You may discover that one or more of your cluster nodes are offline in a number of ways, including the `recovery partition states` [CLI command](../ignite-cli-tool.md#disaster-recovery-commands) with the `--global` option, which would show `Read-only partition`, `Degraded partition`, or `Unavailable partition` for offline nodes.
...
If the node(s) that remain(s) online include the primary replica, the partition becomes `Read-only partition` (see [Global Partition States](#global-partition-states)); all the data is available for reading until lease expires.
