---
title: Getting Started With Apache Ignite 3
...
We will be using the [zip archive](../installation/installing-using-zip.md) to demonstrate how to use Ignite. When using [deb or rpm packages](../installation/deb-rpm.md), or when running Ignite in Docker, some steps may be different.

If you are more comfortable with running the database from Java code, you can try [starting Ignite from code](embedded-mode.md).
...
2. Also from the same page, download the [Ignite command line interface](../ignite-cli-tool.md). This tool is the main way of interacting with Ignite database and will be used in the tutorial.
...
The primary means of interacting with your nodes and cluster is the [Ignite CLI](../ignite-cli-tool.md). It can connect to a node running on a local or remote machine, and is the main tool that is used to manually configure and manage the database. In this example, we will be connecting to a local node.
...
Optionally, you can pass the `--metastorage-group` parameter to specify the nodes that will be used to store cluster meta information. In most scenarios, you want to have 3, 5 or 7 metastorage group nodes. For more information on what they are and cluster lifecycle, see [Cluster Lifecycle](../administrators-guide/lifecycle.md).
...
1. Create a `docker-compose.yml` file with the cluster configuration (see [Installing Using Docker](../installation/installing-using-docker.md) for details)
...
- Check out the [Ignite CLI Tool](../ignite-cli-tool.md) page for more detail on supported commands
