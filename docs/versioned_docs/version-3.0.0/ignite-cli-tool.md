---
title: Apache Ignite CLI Tool
sidebar_label: Ignite CLI Tool
---
...
To use the CLI in the interactive mode, first [run](quick-start/getting-started-guide.md#start-the-ignite-cli) it, then configure the [cluster](administrators-guide/config/cluster-config.md) or [node](administrators-guide/config/node-config.md) using the `update` command.
