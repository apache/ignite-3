...
| IDENTITY | Identity to use for authentication. Depending on the authenticator used on the server side, it can be a user name or another unique identifier. See the [Authentication](/docs/3.1.0/administrators-guide/security/authentication) topic for details. | None. |
| SECRET | Secret to use for authentication. Depending on the authenticator used on the server side, it can be a user password or another type of user-specific secret. See the [Authentication](/docs/3.1.0/administrators-guide/security/authentication) topic for details. | None. |
