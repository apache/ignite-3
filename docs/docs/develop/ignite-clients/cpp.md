...
Client connection parameters are controlled by the client connector configuration. By default, Ignite accepts client connections on port 10800. You can change the configuration for the node by using the [CLI tool](/docs/ignite-cli-tool) at any time.
...
| ssl.clientAuth | | Type of client authentication used by clients. For more information, see [SSL/TLS](/docs/administrators-guide/security/ssl-tls). |
...
Ignite 3 is focused on SQL, and SQL API is the primary way to work with the data. You can read more about supported SQL statements in the [SQL Reference](/docs/sql-reference/ddl) section. Here is how you can send SQL requests:
