...
When the transaction is created, the node that the transaction was started from is chosen as **transaction coordinator**. The coordinator finds the required [partitions](/docs/administrators-guide/storage/data-partitions) and sends the read or write requests to the nodes holding primary partitions. For correct transaction operation, all nodes in cluster must have similar time, that can be different by no more than `schemaSync.maxClockSkewMillis`.
