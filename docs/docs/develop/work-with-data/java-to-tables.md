...
While [SQL DDL](/docs/sql-reference/ddl) supports a comprehensive set of table manipulation commands, you can also create tables and build indexes directly from a POJO using a simple Java API.

This API supports custom annotations and simple builders; it works seamlessly with the Mapper interface, thus facilitating [KeyValueView and RecordView](/docs/develop/work-with-data/table-api).
...
:::note
You need to create a [storage profile](/docs/administrators-guide/storage#creating-and-using-storage-profiles) in node configuration by using the CLI tool.
:::
...
Once you have created a table using the Java API, you can manipulate it using the [SQL commands](/docs/sql-reference/ddl).
