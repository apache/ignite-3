...
A storage profile is the Apache Ignite node entity that defines the configuration parameters for a Storage Engine. A [Distribution Zone](/administrators-guide/storage/distribution-zones) must be configured to use a set of Storage Profiles declared in the node configuration. A table can only have a single primary storage profile defined.
...
After defining storage profiles and [distribution zones](/administrators-guide/storage/distribution-zones), you can create tables using SQL or [from code](/developers-guide/table-api). Both zone and storage profile cannot be changed after table creation.
