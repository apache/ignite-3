...
A storage profile is the Apache Ignite node entity that defines the configuration parameters for a Storage Engine. A [Distribution Zone](/docs/3.1.0/administrators-guide/storage/distribution-zones) must be configured to use a set of Storage Profiles declared in the node configuration. A table can only have a single primary storage profile defined.
...
After defining storage profiles and [distribution zones](/docs/3.1.0/administrators-guide/storage/distribution-zones), you can create tables using SQL or [from code](/docs/3.1.0/developers-guide/table-api). Both zone and storage profile cannot be changed after table creation.
