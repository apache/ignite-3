...
- `having_boolean_expression` - an SQL expression that is run against groups and returns a boolean value. Can use [aggregate functions](/docs/3.1.0/sql/reference/data-types-and-functions/operators-and-functions#aggregate-functions). Only the groups for which `TRUE` was returned will be returned. If not specified, all matching groups are returned.
