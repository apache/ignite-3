...
Another way to cancel queries is by using the SQL [KILL QUERY](/docs/3.1.0/sql/reference/operational-commands#kill-query) command. The query id can be retrieved via the `SQL_QUERIES` [system view](/docs/3.1.0/configure-and-operate/monitoring/metrics-system-views).
