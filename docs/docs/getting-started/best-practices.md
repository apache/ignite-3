...
Ignite stores data in off-heap memory, reserved individually for each [storage engine](/docs/3.1.0/administrators-guide/config/storage/persistent) as required. However, Java Heap memory is still used to handle intermediary objects generated by workloads. For example:

* Cluster and node metadata. This includes information about what nodes are part of the cluster, internal logs, table [version chains](/docs/3.1.0/administrators-guide/storage/data-partitions#version-storage), what keys are locked for transactions, and all other information required for Ignite to operate normally.
...
* [Compute](/docs/3.1.0/developers-guide/compute/compute) operations are likely to use heap memory to store data. Specific requirements for compute jobs vary depending on what job is being performed.
...
:::note
You can also configure client logging for the [Java client](/docs/3.1.0/developers-guide/clients/java#logging).
:::
