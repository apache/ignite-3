...
To start an Ignite node, you will need an Ignite configuration file that specifies all configuration properties of the node. For this tutorial, we recommend [installing](/docs/administrators-guide/install/zip-archive) Ignite 3 and using a default configuration file from it. This file is stored in the `ignite3-db-3.0.0/etc/ignite-config.conf` file.
...
* Check out the [Developers guide](/docs/developers-guide/table-api) page for more information on available APIs
