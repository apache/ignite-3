...
By default, rest connector starts on port 10300. This port can be configured in the `ignite.rest` [node configuration](/administrators-guide/config/node-config#rest-configuration).
...
You are not limited to only monitoring, as Apache Ignite REST API provides endpoints that can be used to manage the cluster as well. For example, you can create a [snapshot](/administrators-guide/snapshots/snapshots-and-recovery) via REST:
