...
By default, all users can perform any updates on the cluster, or [upload arbitrary code to the cluster](/docs/developers-guide/code-deployment/code-deployment) and perform remote code execution with [distributed computing](/docs/developers-guide/compute/compute). To improve security, we recommend configuring user roles and permissions and enabling authorization on the cluster.
...
- Enable SSL for cluster communication with the `ignite.network.ssl` [node configuration](/docs/configure-and-operate/reference/node-configuration).
...
- Enable SSL for client communication with the `ignite.clientConnector.ssl` [node configuration](/docs/configure-and-operate/reference/node-configuration).
- Enable [authentication](/docs/configure-and-operate/configuration/authentication) on the cluster.
