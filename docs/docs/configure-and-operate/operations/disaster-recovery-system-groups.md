...
- [Cluster Management Group (CMG)](/docs/configure-and-operate/operations/lifecycle#cluster-management-group)
- [Metastorage Group (MG)](/docs/configure-and-operate/operations/lifecycle#cluster-metastorage-group)
...
:::note
Disaster recovery for [data partitions](/docs/configure-and-operate/operations/disaster-recovery-partitions) is described in a separate page.
:::
...
2. If the above fails, forcefully assign a new majority using the following [CLI command](/docs/developers-guide/ignite-cli-tool#disaster-recovery-commands) (manually or via REST): `recovery cluster reset --url=<node-url> --cluster-management-group=<new-cmg-nodes>`.
...
3. If some nodes were down or were unavailable due to a network partition (and hence did not participate in the repair):
   ...
   2. Migrate these nodes to the repaired cluster using the following [CLI command](/docs/developers-guide/ignite-cli-tool#disaster-recovery-commands) (manually or via REST): `recovery cluster migrate --old-cluster-url=<url-of-old-cluster-node> --new-cluster-url=<url-of-new-cluster-node>`.
...
2. If the above fails:
   ...
   2. Forcefully assign a new majority using the following [CLI command](/docs/developers-guide/ignite-cli-tool#disaster-recovery-commands) (manually or via REST): `recovery cluster reset --url=<existing-node-url> [--cluster-management-group=<new-cmg-nodes>] --metastorage-replication-factor=N`.
