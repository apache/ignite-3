...
You may discover that one or more of your cluster nodes are offline in a number of ways, including the `recovery partition states` [CLI command](/docs/3.1.0/developers-guide/ignite-cli-tool#disaster-recovery-commands) with the `--global` option, which would show `Read-only partition`, `Degraded partition`, or `Unavailable partition` for offline nodes.
