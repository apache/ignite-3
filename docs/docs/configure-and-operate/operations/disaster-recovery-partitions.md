...
:::note
Disaster recovery for system groups, [Cluster Management Group](/docs/configure-and-operate/operations/disaster-recovery-system-groups#cluster-management-group) and [Metastorage Group](/docs/configure-and-operate/operations/disaster-recovery-system-groups#metastorage-group), is described in a separate page.
:::
...
You may discover that one or more of your cluster nodes are offline in a number of ways, including the `recovery partition states` [CLI command](/docs/developers-guide/ignite-cli-tool#disaster-recovery-commands) with the `--global` option, which would show `Read-only partition`, `Degraded partition`, or `Unavailable partition` for offline nodes.
