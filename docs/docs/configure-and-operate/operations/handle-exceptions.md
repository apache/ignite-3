...
| `IGN-META-7` | Failed to start a node because metastorage has diverged as a result of [MG recovery](/3.1.0/configure-and-operate/operations/disaster-recovery-system-groups#metastorage-group). | Clear the node of data and add it to the cluster as a blank node. |
