...
NOTE: Using the link:installing-with-helm[Helm chart] is recommended, however, if you choose not to use Helm, this guide will walk you through installing Apache Ignite on Kubernetes.

== Prerequisites

=== Recommended Kubernetes Version

include::includes/prereqs-k8s.adoc[]

== Installation Steps

=== Create ConfigMaps

. Create the Apache Ignite configuration file. The minimum node configuration is as follows:
+
.ignite-config.conf
[source,json]
----
...
----
+
. Create the ConfigMap object for Apache Ignite configuration:
+
[source,shell]
----
...
+
NOTE: In Kubernetes deployments, the `ignite-config.conf` file is mounted as a read-only ConfigMap, so any attempt to update it with the `node config update` command will fail.
+
...
+
If your environments does not support `LoadBalancer`, you can use `type: NodePort` instead. Refer to the Kubernetes link:https://kubernetes.io/docs/tasks/access-application-cluster/create-external-load-balancer/[documentation] for details.
+
[source, yaml]
----
apiVersion: v1
...
----
+
. Apply the `statefulset.yaml` file to deploy the main components of Apache Ignite 3:
+
[source,shell]
----
