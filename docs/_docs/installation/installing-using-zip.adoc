...
- `ignite3-db-{version}` - this archive contains everything related to the Ignite database, and when unpacked,  the folder where data will be stored by default. You start Ignite nodes from this folder.
- `ignite3-cli-{version}` - this archive contains the link:ignite-cli-tool[Ignite CLI tool]. This tool is the main way of interacting with Ignite clusters and nodes.
