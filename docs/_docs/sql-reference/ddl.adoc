...
* `USING` - specifies whether the command creates a B+ Tree index or a hash index. Possible values: `HASH`, `TREE`.
* `column_name` - specifies whether column names will be sorted in ascending or descending order. Possible values: `ASC`, `DESC`.
