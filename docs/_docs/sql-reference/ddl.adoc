...
* `ZONE` - sets the link:sql-reference/distribution-zones[Distribution Zone]. Can be specified as a case-sensitive string or case-insensitive identifier. Does not need to exist at the moment of table creation, and can be created before writing data.
