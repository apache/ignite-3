...
* As for now, Apache Ignite can only apply partition awareness optimization for queries over single partition of a single table.

* DML queries, which can **not** be rewritten to key-value operations, are not supported yet. +
...
----
* or a similar `IndexScan` that search bound has a prefix of all the colocation columns:
----
IndexScan
      table: PUBLIC.T
      predicate: AND(=(COLOCATION_COL_1, ?), =(COLOCATION_COL_2, ?))
