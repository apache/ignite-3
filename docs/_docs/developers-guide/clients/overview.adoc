...
* Equality on affinity columns required
    Partition awareness works only if the SQL query contains an equality condition on all colocated (affinity) columns. This is required for the client to route SQL query to the node where the partition with the data resides.
...
The resulting plan should contain a TableScan or IndexScan node with conditions on all colocation (affinity) columns of the table.
...
In other more complex cases (nested queries, IN condition, JOINS), partition awareness may work or may not work that depends whether the query can be rewritten in form that contains colocation information.
