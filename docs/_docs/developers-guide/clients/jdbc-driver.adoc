...
** `reconnectInterval` - Background reconnect interval, in milliseconds. `0` means that background reconnect is disabled. Default value: `30_000`.
** `reconnectRetriesLimit` - Maximum number of retry attempts to establish connection. `0` means that no retries will be made. `-1` means that the number of retries is not limited. Default value: `16`.
