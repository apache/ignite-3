...
** `backgroundReconnectIntervalMillis` - Specifies the wait time in milliseconds between runs of the background reconnection procedure. This procedure is used to restore old and establish new connections to cluster nodes. The value `0` can be used to disable background reconnection. Default value is `30000` (30 seconds).
