...
** `backgroundReconnectInterval` - Background reconnect interval, in milliseconds. The value `0` can be used to disable background reconnection. Default value is `30000` (30 seconds).
Background reconnection is used to restore old and establish new connections to cluster nodes in the background.
For example, if you disable it, the client will not try to restore connections to temporarily unavailable cluster nodes.
** `reconnectRetriesLimit` - Maximum number of *retry* attempts to establish connection. The value `0` means that no retries will be made. The value `-1` means that the number of retries is not limited. Default value is `16`.
This value sets the maximum number of retries to connect to cluster nodes. For example, in a 3-node cluster, the driver connects to the first node, but for some reason it is unavailable. If the `reconnectRetriesLimit` is zero, the driver will not attempt to connect to the second node and will immediately return a connection error.
