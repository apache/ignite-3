...
When it is enabled for the data region, Ignite stores all the data on disk, and loads as much data as it can into RAM for processing.
...
When persistence is enabled, Apache Ignite stores each partition in a separate file on disk.
If partition backups are enabled, they are also saved on disk.
In addition to data partitions, Apache Ignite stores indexes and metadata.
