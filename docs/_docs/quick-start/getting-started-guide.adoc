...
----
unzip ignite3-db-{version}.zip
unzip ignite3-cli-{version}.zip
----

tab:Windows (PowerShell)[]
[source,shell, subs="attributes,specialchars"]
----
Expand-Archive ignite3-db-{version}.zip -DestinationPath .
Expand-Archive ignite3-cli-{version}.zip -DestinationPath .
----

tab:Windows (CMD)[]
[source,shell, subs="attributes,specialchars"]
----
unzip -xf ignite3-db-{version}.zip
unzip -xf ignite3-cli-{version}.zip
----
--

Now you should have the `ignite3-db-{version}` and `ignite3-cli-{version}` directories that we will be using in this tutorial, and the license file provided via e-mail.
...
. Navigate to the `ignite3-db-{version}` directory.
...
. Navigate to the `ignite3-cli-{version}` directory.
...
tab:Command[]
----
cluster init --name=sampleCluster --metastorage-group=defaultNode --config-files=valid-config.conf
----
...
* The `--config-files` parameter specifies the relative path to the link:administrators-guide/config/cluster-config[cluster configuration] file. Any parameters not specified in the configuration will be assigned default values. The relative path to the configuration file is resolved based on the directory the CLI tool is started from. You can also provide the absolute path to the file.
+
...
[source,shell]
----
docker pull ignite/ignite3:latest
----
...
[source,shell]
----
cluster init --name=sampleCluster --metastorage-group=node1 --config-files=valid-config.conf
----
