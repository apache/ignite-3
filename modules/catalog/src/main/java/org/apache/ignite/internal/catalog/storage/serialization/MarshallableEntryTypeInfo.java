 ...
 * <pre>
 *     var typeInfo = MarshallableEntryTypeInfo.type(MyClass.class, typeId, serializerVersion);
 * </pre>
 *
 * Type information for a field or a collection that stores elements of different types.
 * <pre>
 *     var typeInfo = MarshallableEntryTypeInfo.type(BaseClass.class)
 *       .addVariant(MarshallableEntry.VARIANT_1, 2)
 *       .addVariant(MarshallableEntry.VARIANT_2, 3)
 *       .build;
 * </pre>
 *
 * @param <T> Type.
 */
public final class MarshallableEntryTypeInfo<T> {

    private final Class<T> type;

    private final Int2IntMap types;

    private MarshallableEntryTypeInfo(Class<T> type, Int2IntMap versions) {
        ...
        this.type = type;
        this.types = new Int2IntOpenHashMap(versions);
    }
    ...
    public int version(int typeId) {
        int version = types.get(typeId);
        if (version == 0) {
            throw new IllegalArgumentException(format("Unexpected typeId: {}. Registered: {}"));
        }
        return version;
    }
     ...
     *
     */
    public static <T> MarshallableEntryTypeInfo<T> typeOf(Class<T> entryType, MarshallableEntryType type, int version) {
        return new MarshallableEntryTypeInfo<>(entryType, Int2IntMaps.singleton(type.id(), version));
    }
    ...
    public static class Builder<T> {
         ...
         */
        public MarshallableEntryTypeInfo<T> build() {
            return new MarshallableEntryTypeInfo<>(type, versions);
        }
    }
}
