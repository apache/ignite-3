 ...
 */
public class CatalogCompactionRunnerImpl implements CatalogCompactionRunner {
    /** The logger. */
    private static final IgniteLogger LOG = Loggers.forClass(CatalogCompactionRunnerImpl.class);

    private volatile CompletableFuture<Boolean> opState = CompletableFutures.nullCompletedFuture();
     ...
     */
    public CatalogCompactionRunnerImpl(
    ...
    public CompletableFuture<Void> startAsync(ComponentContext componentContext) {
        messagingService.addMessageHandler(CatalogMessageGroup.class, (message, sender, correlationId) -> {
            ...
            assert message.messageType() == CatalogMessageGroup.MINIMUM_REQUIRED_TIME_REQUEST;
    ...
    /** Called when low watermark is changed. */
    ...
    @TestOnly
    CompletableFuture<Boolean> operationStateFuture() {
        return opState;
    }

    /** Initiates catalog compaction routine. */
    CompletableFuture<Boolean> startCompaction(HybridTimestamp newLowWatermark) {
        CompletableFuture<Boolean> fut = opState;
    ...
    private CompletableFuture<Boolean> startCompaction() {
        return resolveMinimumRequiredTimeInCluster().thenCompose(ts -> {
