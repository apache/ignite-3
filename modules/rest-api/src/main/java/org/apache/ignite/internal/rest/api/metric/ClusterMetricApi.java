...
public interface ClusterMetricApi {
    ...
    @Post("enable")
    CompletableFuture<Void> enable(@Body String srcName);
    ...
    @Produces(MediaType.PROBLEM_JSON)
    @Post("disable")
    CompletableFuture<Void> disable(@Body String srcName);
    ...
    @ApiResponse(responseCode = "200", description = "Returned a list of metric sources.",
            content = @Content(mediaType = MediaType.APPLICATION_JSON,
                    array = @ArraySchema(schema = @Schema(implementation = NodeMetricSources.class))))
    @ApiResponse(responseCode = "500", description = "Internal error.",
            content = @Content(mediaType = MediaType.PROBLEM_JSON, schema = @Schema(implementation = Problem.class)))
    @Get("source")
