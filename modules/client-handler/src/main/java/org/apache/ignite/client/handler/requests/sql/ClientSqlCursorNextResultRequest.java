...
public class ClientSqlCursorNextResultRequest {
    ...
    public static CompletableFuture<ResponseWriter> process(
        ...
        f.whenCompleteAsync((r, t) -> {
            if (t != null) {
                cursorWithPageSize.cursorFuture().thenAccept(cur -> closeRemainingCursors(cur, true, operationExecutor));
            }
        }, operationExecutor);
    ...
    private static void closeRemainingCursors(AsyncSqlCursor<?> cursor, boolean first, Executor operationExecutor) {
        if (cursor.hasNextResult()) {
            cursor.nextResult().whenCompleteAsync((c, err) -> {
                if (c != null) {
                    cursor.closeAsync();
                    closeRemainingCursors(c, false, operationExecutor);
                }
            }, operationExecutor);
        } else if (!first) {
            cursor.closeAsync();
