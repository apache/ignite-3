...
/**
 * REST exception handler for {@link ConfigurationValidationIgniteException}.
 */
@Singleton
...
public class ConfigurationValidationExceptionHandler
    ...
    public HttpResponse<? extends Problem> handle(HttpRequest request, ConfigurationValidationIgniteException exception) {
        return HttpProblemResponse.from(
                Problem.fromIgniteException(exception, HttpCode.BAD_REQUEST)
                        .invalidParams(mapValidationIssuesToRestFormat(exception))
                        .build()
        );
    }
