...
import org.apache.ignite.internal.network.file.messages.FilesMetadataImpl;
...
public class ItFileTransferTest {
    ...
    void download() throws IOException {
        ...
        CompletableFuture<List<File>> downloadedFilesFuture = node1.fileTransferringService().download(
                node0.nodeName(),
                FilesMetadataImpl.builder().build()
        );
    ...
    void upload() throws IOException {
        ...
        node1.fileTransferringService().addFileHandler(FilesMetadata.class, ((metadata, uploadedFilesDir) -> {
            uploadedFilesFuture.complete(uploadedFilesDir);
            return completedFuture(null);
        }));

        node0.fileTransferringService().upload(node1.nodeName(), FilesMetadataImpl.builder().build());
    ...
    void uploadFilesWhenProviderThrowsException() {
        ...
        CompletableFuture<List<File>> uploadedFilesFuture = new CompletableFuture<>();
        node1.fileTransferringService().addFileHandler(FilesMetadata.class, ((metadata, uploadedFiles) -> {
