...
public class MappedFile implements Closeable, DirectMemoryRegion {
    private static final Method map0 = findNonPublicMethod(FileChannelImpl.class, "map0", int.class, long.class, long.class);

    private static final Method unmap0 = findNonPublicMethod(FileChannelImpl.class, "unmap0", long.class, long.class);
     ...
     * @param size Expected file size. If size is {@code 0}, the existing size will be used.
    ...
    /**
     * Returns underlying random access file.
     */
    ...
    public DirectMemoryRegion slice(long offset) {
        if (offset < 0 || offset >= size) {
            throw new IllegalArgumentException("Failed to create a memory region slice [ptr=" + hexLong(addr)
     ...
     */
    private static void unmap(long addr, long size) {
        try {
