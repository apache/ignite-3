...
/**
 * Represents speed calculation of some progress starting from zero. This class not only takes into account current progress value, but also
 * 3 previous values, each of them is pushed to history when {@link #closeInterval()} is called.
 */
class ProgressSpeedCalculation {
    ...
    /**
     * Closes the current interval by pushing it to the history. The 3 latest intervals put into history affect the average speed
     * calculation via {@link #getOpsPerSecond(long)} and {@link #getOpsPerSecondReadOnly()}.
     */
