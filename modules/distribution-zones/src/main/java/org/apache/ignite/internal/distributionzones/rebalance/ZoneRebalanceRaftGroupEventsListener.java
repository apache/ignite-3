...
public class ZoneRebalanceRaftGroupEventsListener implements RaftGroupEventsListener {
    ...
    /** {@inheritDoc} */
    @Override
    public void onReconfigurationError(Status status, PeersAndLearners configuration, long term, long casualityToken) {
        if (!busyLock.enterBusy()) {
            return;
        }
        ...
        try {
            ...
            if (rebalanceAttempts.incrementAndGet() < REBALANCE_RETRY_THRESHOLD) {
                scheduleChangePeersAndLearners(configuration, term, casualityToken);
            } else {
