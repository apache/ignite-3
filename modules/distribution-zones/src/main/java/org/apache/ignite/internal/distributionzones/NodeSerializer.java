...
public class NodeSerializer extends VersionedSerializer<Node> {
    ...
    @Override
    protected void writeExternalData(Node node, IgniteDataOutput out) throws IOException {
        out.writeUTF(node.nodeName());
        out.writeUuid(node.nodeId());
    }

    @Override
    protected Node readExternalData(byte protoVer, IgniteDataInput in) throws IOException {
