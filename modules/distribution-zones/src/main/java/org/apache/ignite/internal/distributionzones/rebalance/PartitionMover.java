...
public class PartitionMover {
     ...
     */
    public CompletableFuture<Void> movePartition(PeersAndLearners peersAndLearners, long term, long casualityToken) {
        ...
        try {
            return raftGroupServiceSupplier
                    ...
                    .thenCompose(raftGroupService ->
                            raftGroupService.changePeersAndLearnersAsync(peersAndLearners, term, casualityToken))
                    .handle((resp, err) -> {
                        if (!busyLock.enterBusy()) {
                            throw new IgniteInternalException(NODE_STOPPING_ERR, new NodeStoppingException());
                        }
                        ...
                        try {
                            if (err != null) {
                                ...
                                rebalanceScheduler.schedule(() -> {
                                    movePartition(peersAndLearners, term, casualityToken).whenComplete(copyStateTo(future));
