...
public interface Node extends Lifecycle<NodeOptions>, Describer {
     ...
     * <p>
     * About the ownership: |task.data|: for the performance consideration, we will take away the content. If you want keep the content,
     * copy it before call this function |task.done|: If the data is successfully committed to the raft group. We will pass the ownership to
     * #{@link StateMachine#onApply(Iterator)}. Otherwise we will specify the error and call it.
     *
     ...
     * [Thread-safe and wait-free]
     * <p>
     * Starts a linearizable read-only query request with request context(optional, such as request id etc.) and closure.  The closure will
     * be called when the request is completed, and user can read data from state machine if the result status is OK.
     *
     ...
     * <p>
     * [NOTE] <strong>when list_peers concurrency with {@link #addPeer(PeerId, Closure)}/{@link #removePeer(PeerId, Closure)}, maybe return
     * peers is staled.  Because {@link #addPeer(PeerId, Closure)}/{@link #removePeer(PeerId, Closure)} immediately modify configuration in
     * memory</strong>
     *
     ...
     * <p>
     * [NOTE] <strong>list_alive_peers is just a transient data (snapshot) and a short-term loss of response by the follower will cause it
     * to temporarily not exist in this list.</strong>
     *
     ...
     * <p>
     * [NOTE] <strong>when listLearners concurrency with {@link #addLearners(List, Closure)}/{@link #removeLearners(List, Closure)}/{@link
     * #resetLearners(List, Closure)}, maybe return peers is staled.  Because {@link #addLearners(List, Closure)}/{@link
     * #removeLearners(List, Closure)}/{@link #resetLearners(List, Closure)} immediately modify configuration in memory</strong>
     *
     ...
     * <p>
     * [NOTE] <strong>when listAliveLearners concurrency with {@link #addLearners(List, Closure)}/{@link #removeLearners(List,
     * Closure)}/{@link #resetLearners(List, Closure)}, maybe return peers is staled.  Because {@link #addLearners(List, Closure)}/{@link
     * #removeLearners(List, Closure)}/{@link #resetLearners(List, Closure)} immediately modify configuration in memory</strong>
     *
    ...
    /**
     * Change the configuration of the raft group to |newPeers| , done.run() would be invoked after this operation finishes, describing the
     * detailed result.
     *
    ...
    /**
     * Reset the configuration of this node individually, without any replication to other peers before this node becomes the leader. This
     * function is supposed to be invoked when the majority of the replication group are dead and you'd like to revive the service in the
     * consideration of availability. Notice that neither consistency nor consensus are guaranteed in this case, BE CAREFULE when dealing
     * with this method.
     *
    ...
    /**
     * Try transferring leadership to |peer|. If peer is ANY_PEER, a proper follower will be chosen as the leader for the next term. Returns
    ...
    /**
     * Read the first committed user log from the given index. Return OK on success and user_log is assigned with the very data. Be awared
     * that the user_log may be not the exact log at the given index, but the first available user log from the given index to
     * lastCommittedIndex. Otherwise, appropriate errors are returned: - return ELOGDELETED when the log has been deleted; - return
     * ENOMOREUSERLOG when we can't get a user log even reaching lastCommittedIndex. [NOTE] in consideration of safety, we use
     * lastAppliedIndex instead of lastCommittedIndex in code implementation.
     *
    ...
    /**
     * JRaft users can implement the ReplicatorStateListener interface by themselves. So users can do their own logical operator in this
     * listener when replicator created, destroyed or had some errors.
     *
