...
/**
 * V2 log entry encoder. Extends V1 with sequence token support.
 */
public final class V2Encoder implements LogEntryEncoder {
    private V2Encoder() {
    }
    ...
    public int size(LogEntry logEntry) {
        ...
        int totalLen = LogEntryV2CodecFactory.PAYLOAD_OFFSET;
        int typeNumber = type.getNumber();
        long index = id.getIndex();
        long term = id.getTerm();
        long sequenceToken = logEntry.getSequenceToken();
        long oldSequenceToken = logEntry.getOldSequenceToken();

        // Checksum is not a varlen value.
        totalLen += sizeInBytes(typeNumber) + sizeInBytes(index) + sizeInBytes(term) + Long.BYTES;

        // V2: Add sequence tokens
        totalLen += sizeInBytes(sequenceToken) + sizeInBytes(oldSequenceToken);
    ...
    public void append(long addr, LogEntry logEntry) {
        ...
        GridUnsafe.putByte(addr++, LogEntryV2CodecFactory.MAGIC);
        ...
        // V2: Write sequence tokens
        addr += writeLong(logEntry.getSequenceToken(), addr);
        addr += writeLong(logEntry.getOldSequenceToken(), addr);
    ...
    public void encode(ByteBuffer buffer, LogEntry log) {
        ...
        buffer.put(LogEntryV2CodecFactory.MAGIC);
        ...
        // V2: Write sequence tokens
        writeLong(log.getSequenceToken(), buffer);
        writeLong(log.getOldSequenceToken(), buffer);
