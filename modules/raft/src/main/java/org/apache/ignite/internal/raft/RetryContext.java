...
class RetryContext {
     ...
     * @param originDescription Supplier describing the origin request from which this one depends,
     *        or returning {@code null} if this request is independent.
     ...
     *        <ul>
     *            <li>{@code 0} — no retries are performed; a single attempt is made to send the
     *            replication request.</li>
     *            <li>Any negative value or {@code Long.MAX_VALUE} — retries are unbounded; attempts
     *            continue indefinitely until the request succeeds or the service becomes ready.</li>
     *            <li>Any positive value — retries continue until the timeout expires.</li>
     *        </ul>
     * @param responseTimeoutMillis Response timeout for each individual attempt (up to the retry
     *        deadline), in milliseconds, or {@code -1} to use
    ...
    RetryContext(
        ...
        // It's essential that we initialize `stopTime` before `startTime` to cover one-shot cases.
    ...
    boolean isRetryChainTimeout(long requestMonotonicTime) {
        return !isOneShot() && requestMonotonicTime >= stopTime;
    }
