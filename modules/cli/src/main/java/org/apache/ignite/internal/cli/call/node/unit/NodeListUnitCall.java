...
/** List units on the node call. */
@Singleton
public class NodeListUnitCall extends ListUnitCall {
    private final ApiClientFactory clientFactory;

    public NodeListUnitCall(ApiClientFactory clientFactory) {
    ...
    protected List<UnitStatus> getStatuses(ListUnitCallInput input) throws ApiException {
        ...
        if (input.unitId() != null) {
            return api.listNodeStatusesByUnit(input.unitId(), input.version(), input.statuses());
        } else {
            return api.listNodeStatuses(input.statuses());
        }
    }
}
