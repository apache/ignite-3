...
public interface IgniteCompute {
    ...
    default <R> Map<ClusterNode, R> executeBroadcast(
        ...
        return nodes.stream()
                .collect(toUnmodifiableMap(identity(), node -> execute(Set.of(node), units, jobClassName, options, args)));
    }

    /**
     * Executues a {@link ComputeJob} of the given class on all nodes in the given node set
     * with default execution options {@link JobExecutionOptions#DEFAULT}.
     *
     * @param nodes Nodes to execute the job on.
     * @param units Deployment units. Can be empty.
