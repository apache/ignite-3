...
public class RocksDBExample {
    ...
    public static void main(String[] args) {
        ...
        try (final Options options = new Options();
            ...
            try (final RocksDB db = RocksDB.open(options, db_path)) {
                ColumnFamilyHandle handle = db.createColumnFamily(new ColumnFamilyDescriptor(new byte[]{0}));

                db.put(handle, "hello".getBytes(), "world".getBytes());
                byte[] value = db.get(handle, "hello".getBytes());
                ...
                try (final RocksIterator iterator = db.newIterator()) {
                    ...
                    RocksIterator i = db.newIterator(new ReadOptions().setSnapshot(snapshot));
                    i.close();
