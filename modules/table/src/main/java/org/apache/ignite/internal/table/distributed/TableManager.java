...
public class TableManager implements IgniteTablesInternal, IgniteComponent {
    ...
    private CompletableFuture<Boolean> onPrimaryReplicaExpired(PrimaryReplicaEventParameters parameters) {
        if (topologyService.localMember().id().equals(parameters.leaseholderId())) {
            var groupId = (ZonePartitionId) parameters.groupId();

            replicaMgr.weakStopReplica(
                    groupId,
                    WeakReplicaStopReason.PRIMARY_EXPIRED,
                    () -> stopAndDestroyPartition(
                            groupId,
    ...
    private CompletableFuture<Void> startPartitionAndStartClient(
        ...
        Supplier<RaftGroupService> getCachedRaftClient = () -> {
            try {
                ...
                return internalTbl
                        .tableRaftService()
                        .partitionRaftGroupService(partId);
        ...
        if (localMemberAssignment != null) {
            ...
            startGroupFut = replicaMgr.weakStartReplica(
                    ...
                    () -> shouldStartGroupFut.thenComposeAsync(startGroup -> inBusyLock(busyLock, () -> {
                        ...
                        SnapshotStorageFactory snapshotStorageFactory = createSnapshotStorageFactory(zonePartitionId,
                        ...
                        RaftGroupEventsListener raftGroupEventsListener = createRaftGroupEventsListener(tableId, zonePartitionId);
    ...
    private RaftGroupEventsListener createRaftGroupEventsListener(int tableId, ZonePartitionId zonePartitionId) {
        PartitionMover partitionMover = createPartitionMover(zonePartitionId);
                        ...
                        return setTablesPartitionCountersForRebalance(zonePartitionId, revision, pendingAssignments.force())
                                .thenCompose(r -> {
                                    ...
                                    for (int tableId : tablesInZone) {
                                        ZonePartitionId fullZonePartitionId = new ZonePartitionId(zoneId, tableId, partId);
                                        ...
                                        if (LOG.isInfoEnabled()) {
                                            ...
                                            LOG.info(
                                                    "Received update on pending assignments. Check if new raft group should be started"
    ...
    private CompletableFuture<Void> handleChangePendingAssignmentEventForTable(
        ...
        if (shouldStartLocalGroupNode) {
            ...
            localServicesStartFuture = runAsync(() -> {
                if (pendingAssignmentsAreForced && replicaMgr.isReplicaStarted(zonePartitionId)) {
                    replicaMgr.resetPeers(zonePartitionId, fromAssignments(nonStableNodeAssignmentsFinal.nodes()));
                }
            }, ioExecutor);
        }
    ...
    private SnapshotStorageFactory createSnapshotStorageFactory(
        ...
        PartitionKey partitionKey = partitionKey(internalTable, zonePartitionId.partitionId());
        ...
        return supplyAsync(() -> {
            ...
            if (LOG.isInfoEnabled()) {
                ...
                LOG.info("Received update on stable assignments [key={}, partition={}, localMemberAddress={}, "
                                + "stableAssignments={}, pendingAssignments={}, revision={}]", stringKey, zonePartitionId,
            ...
            for (Integer tableId : tablesInZone) {
                var zoneTablePartitionId = new ZonePartitionId(zoneId, tableId, partitionId);
    ...
    private CompletableFuture<Void> updatePartitionClients(
        ...
        return tablesById(revision).thenAccept(t -> {
            t.get(zonePartId.tableId()).internalTable()
                    .tableRaftService()
                    .partitionRaftGroupService(zonePartId.partitionId())
    ...
    private CompletableFuture<Void> stopAndDestroyPartitionAndUpdateClients(
            ZonePartitionId tablePartitionId,
            Set<Assignment> stableAssignments,
            Assignments pendingAssignments,
            boolean isRecovery,
            long revision
    ...
    private CompletableFuture<Void> weakStopAndDestroyPartition(ZonePartitionId zonePartitionId, long causalityToken) {
        return replicaMgr.weakStopReplica(
                zonePartitionId,
                WeakReplicaStopReason.EXCLUDED_FROM_ASSIGNMENTS,
                () -> stopAndDestroyPartition(zonePartitionId, causalityToken)
        );
    }

    private CompletableFuture<Void> stopAndDestroyPartition(ZonePartitionId zonePartitionId, long causalityToken) {
        return tablesVv
                .get(causalityToken)
                .thenCompose(ignore -> {
                    TableImpl table = tables.get(zonePartitionId.tableId());

                    return stopPartition(
                            new TablePartitionId(zonePartitionId.tableId(), zonePartitionId.partitionId()),
                            table
                    ).thenComposeAsync(v -> destroyPartitionStorages(zonePartitionId.partitionId(), table), ioExecutor);
                });
    }
     ...
     *
     * @param groupId Partition ID.
     * @param table Table which this partition belongs to.
     * @return Future that will be completed after all resources have been closed.
     */
    private CompletableFuture<Void> stopPartitionForRestart(ZonePartitionId groupId, TableImpl table) {
        return replicaMgr.weakStopReplica(groupId, WeakReplicaStopReason.RESTART, () -> {
            var tablePartitionId = new TablePartitionId(groupId.tableId(), groupId.partitionId());
    ...
    public CompletableFuture<Void> restartPartition(TablePartitionId tablePartitionId, long revision) {
        return inBusyLockAsync(busyLock, () -> tablesVv.get(revision).thenComposeAsync(unused -> inBusyLockAsync(busyLock, () -> {
            ...
            ZonePartitionId zoneTablePartitionId = new ZonePartitionId(
                    zoneId,
                    tablePartitionId.tableId(),
                    tablePartitionId.partitionId()
            );

            return stopPartitionForRestart(zoneTablePartitionId, table).thenComposeAsync(unused1 -> {
                Assignments stableAssignments = stableAssignments(zoneTablePartitionId, revision);

                assert stableAssignments != null : "tablePartitionId=" + tablePartitionId + ", revision=" + revision;

                return startPartitionAndStartClient(
                        table,
                        zoneTablePartitionId.partitionId(),
                        stableAssignments,
                        null,
                        zoneId,
                        false
