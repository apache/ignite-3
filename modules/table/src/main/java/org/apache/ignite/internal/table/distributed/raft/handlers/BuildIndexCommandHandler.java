...
public class BuildIndexCommandHandler extends AbstractCommandHandler<BuildIndexCommand> {
    ...
    protected CommandResult handleInternally(
        ...
        // If storage engine needs resources, just quit. We will retry later.
        if (shouldRelease) {
            return EMPTY_NOT_APPLIED_RESULT;
        }
     ...
     * It is used to signal an early exit when {@link Locker#shouldRelease()} returns {@code true}.
     */
    private static class ShouldReleaseException extends RuntimeException {
        private static final long serialVersionUID = 7471449153661955685L;
    }
}
