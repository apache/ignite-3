...
public class TablePartitionProcessor implements RaftTableProcessor {
    ...
    private <T extends UpdateCommandBase> CommandResult handleAnUpdateCommand(
        ...
        if (shouldUpdateStorage(cmd.full(), storageLeaseInfo)) {
            ...
            failedCompatValidationResult = validateSchemaCompatibilityIfNeeded(txId, cmd.full(), commitTsOrNull);
    ...
    private @Nullable CompatibilityValidationResult validateSchemaCompatibilityIfNeeded(
        ...
        if (!full) {
            // We do not need to validate schema compatibility for full transactions as for them it will be validated on explicit commit.
            return null;
        }

        HybridTimestamp commitTs = Objects.requireNonNull(commitTsOrNull);

        CompletableFuture<CompatibilityValidationResult> validateFuture = schemaCompatibilityValidator
                .validateCommit(txId, Set.of(storage.tableId()), commitTs);
        assert validateFuture.isDone()
                : "Schema information is not ready; make sure replication safeguards not allowing such commands "
                + "to be executed are in place";

        CompatibilityValidationResult validationResult = validateFuture.join();
