 ...
 * <pre><code>
 * [rowId's UUID (16 bytes), txId (16 bytes), commitZoneId (16 bytes), commitPartitionId (2 bytes), head link (6 bytes),
 * next link (6 bytes)]
 * </code></pre>
 */
public interface VersionChainIo {
    ...
    int COMMIT_ZONE_ID_OFFSET = TX_ID_LSB_OFFSET + Long.BYTES;

    /** Offset of commit partition id. */
    int COMMIT_PARTITION_ID_OFFSET = COMMIT_ZONE_ID_OFFSET + Integer.BYTES;
    ...
    default void storeByOffset(long pageAddr, int off, VersionChainKey rowKey) {
        ...
        if (txId == null) {
            ...
            putInt(pageAddr, off + COMMIT_ZONE_ID_OFFSET, commitZoneId);
    ...
    default VersionChain getRow(long pageAddr, int idx, int partitionId) {
        ...
        if (txId != null) {
            int commitZoneId = getInt(pageAddr, offset + COMMIT_ZONE_ID_OFFSET);

            int commitPartitionId = getShort(pageAddr, offset + COMMIT_PARTITION_ID_OFFSET) & 0xFFFF;

            return VersionChain.createUncommitted(rowId, txId, commitZoneId, commitPartitionId, headLink, nextLink);
        }
