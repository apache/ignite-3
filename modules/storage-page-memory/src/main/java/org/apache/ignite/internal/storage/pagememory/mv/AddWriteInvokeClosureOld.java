 ...
 */
class AddWriteInvokeClosureOld implements InvokeClosure<VersionChain> {
    ...
    private RowVersion insertRowVersion(@Nullable BinaryRow row, long nextPartitionlessLink) {
        RowVersion rowVersion = new RowVersion(rowId, storage.partitionId, nextPartitionlessLink, row, 0, 0);
