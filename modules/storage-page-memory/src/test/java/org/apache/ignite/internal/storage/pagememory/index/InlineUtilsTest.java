...
public class InlineUtilsTest {
    ...
    void testInlineSizeForNativeType() {
        ...
        assertEquals(DEFAULT_VARLEN_COLUMN_INLINE_SIZE, inlineSize(nativeType = NativeTypes.stringOf(Integer.MAX_VALUE)));
        nativeTypeSpecs.remove(nativeType.spec());

        assertEquals(9, inlineSize(nativeType = NativeTypes.blobOf(9)));
        nativeTypeSpecs.remove(nativeType.spec());

        assertEquals(DEFAULT_VARLEN_COLUMN_INLINE_SIZE, inlineSize(nativeType = NativeTypes.blobOf(Integer.MAX_VALUE)));
        nativeTypeSpecs.remove(nativeType.spec());
    ...
    @Test
    void testCalculateBinaryTupleInlineSize() {
        IndexDescriptor indexDescriptor = testIndexDescriptor();

        assertEquals(1, calculateBinaryTupleInlineSize(indexDescriptor));
        ...
        assertEquals(22, calculateBinaryTupleInlineSize(indexDescriptor));
        ...
        assertEquals(23, calculateBinaryTupleInlineSize(indexDescriptor));
        ...
        assertEquals(217, calculateBinaryTupleInlineSize(indexDescriptor));

        indexDescriptor = testIndexDescriptor(
                testColumnDescriptor(NativeTypes.stringOf(MAX_INLINE_SIZE), false),
                testColumnDescriptor(NativeTypes.stringOf(MAX_INLINE_SIZE), false)
        );

        assertEquals(MAX_INLINE_SIZE, calculateBinaryTupleInlineSize(indexDescriptor));

        indexDescriptor = testIndexDescriptor(
                testColumnDescriptor(NativeTypes.stringOf(MAX_INLINE_SIZE), true),
                testColumnDescriptor(NativeTypes.stringOf(MAX_INLINE_SIZE), false)
        );
