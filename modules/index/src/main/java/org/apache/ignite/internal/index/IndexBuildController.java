...
class IndexBuildController implements ManuallyCloseable {
    ...
    private void tryScheduleBuildIndexesForNewPrimaryReplica(
        ...
        inBusyLock(busyLock, () -> {
            if (isLeaseExpired(replicaMeta, buildAttemptTimestamp)) {
                LOG.warn("Lease has expired, stopping build index process groupId=[" + primaryReplicaId + "], localNode=[" + localNode() + "], primaryReplciaName=[" + replicaMeta.getLeaseholder() + "], primaryReplciaId=[" + primaryReplicaId + "]");
    ...
    private void tryScheduleBuildIndex(
        ...
        inBusyLock(busyLock, () -> {
            if (isLeaseExpired(replicaMeta, buildAttemptTimestamp)) {
                LOG.warn("Lease has expired 2, stopping build index process groupId=[" + primaryReplicaId + "], localNode=[" + localNode() + "], primaryReplciaName=[" + replicaMeta.getLeaseholder() + "], primaryReplciaId=[" + primaryReplicaId + "]");
                stopBuildingIndexesIfPrimaryExpired(primaryReplicaId);
