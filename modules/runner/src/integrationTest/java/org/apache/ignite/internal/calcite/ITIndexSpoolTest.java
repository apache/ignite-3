...
public class ITIndexSpoolTest extends AbstractBasicIntegrationTest {
    ...
    private Table createTable(String tableName) {
        SchemaTable schTbl1 = SchemaBuilders.tableBuilder("PUBLIC", tableName)
            .columns(
    ...
    private void insertData(Table table, int fromIdx, int cnt) {
        for (int i = fromIdx; i < fromIdx + cnt; i++)
            table.insert(Tuple.create().set("ID", i).set("JID", i + 1).set("VAL", "val_" + i));
    }
