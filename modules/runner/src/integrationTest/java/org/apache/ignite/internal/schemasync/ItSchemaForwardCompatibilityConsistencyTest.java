...
class ItSchemaForwardCompatibilityConsistencyTest extends ClusterPerTestIntegrationTest {
    ...
    private void testForwardIncompatibleSchemaChangesCannotBeCreatedBy1PcTransactions(SinglePartitionOperation operation, int attempt)
        ...
        List<ReadResult> readResults = readRowsFromTable(tableName, rowsAdded.get());
    ...
    private void createTable(String tableName) {
        cluster.doInSession(0, session -> {
            executeUpdate(
                    "CREATE TABLE " + tableName + " (id INT PRIMARY KEY, val INT, column_to_drop INT)",
                    session
            );
        });
    }
    ...
    private void makeExplicitPut(int value, String tableName, SinglePartitionOperation singlePartitionOperation) {
        node.transactions().runInTransaction((Transaction tx) -> makePutInTransaction(value, tx, tableName, singlePartitionOperation));
    }

    private void makePutInTransaction(int value, @Nullable Transaction tx, String tableName, SinglePartitionOperation operation) {
        operation.put(tx, value, value, tableName, node);
    }

    private List<ReadResult> readRowsFromTable(String tableName, int expectedRowCount) {
        List<ReadResult> readResults = readRowsFromTable(tableName);
    ...
    private List<ReadResult> readRowsFromTable(String tableName) {
