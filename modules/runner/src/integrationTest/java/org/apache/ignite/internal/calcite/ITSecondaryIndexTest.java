...
public class ITSecondaryIndexTest extends AbstractBasicIntegrationTest {
    ...
    @Override protected void initTestData() {
        {
            SchemaTable schema = SchemaBuilders.tableBuilder("PUBLIC", "DEVELOPER")
                .columns(
        ...
        {
            SchemaTable schema = SchemaBuilders.tableBuilder("PUBLIC", "UNWRAP_PK")
                .columns(
...
//    }
    ...
    public void testEqualsFilterWithUnwrpKey() {
        assertQuery("SELECT F1 FROM UNWRAP_PK WHERE F2=2")
            .matches(containsIndexScan("PUBLIC", "UNWRAP_PK", PRIMARY_KEY_INDEX_NAME))
            .returns("Ivan")
            .check();
    }
    ...
    public void testKeyEqualsFilter() {
        assertQuery("SELECT * FROM Developer WHERE id=2")
            .matches(containsIndexScan("PUBLIC", "DEVELOPER", PRIMARY_KEY_INDEX_NAME))
            .returns(2, "Beethoven", 2, "Vienna", 44)
            .check();
    }
