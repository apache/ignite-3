...
class ConfigurationBasedChannelRegistryTest extends BaseIgniteAbstractTest {
    ...
    @BeforeEach
    void setUp() {
        registry = new ConfigurationBasedChannelRegistry(
                cfg,
                new ConfigurationBasedSinkRegistry(
                        cfg,
                        new LogSinkFactory(EventSerializerFactory.DEFAULT.createEventSerializer())
                )
        );
    }
