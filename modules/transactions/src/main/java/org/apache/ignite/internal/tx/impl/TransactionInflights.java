...
public class TransactionInflights {
        ...
        for (Map.Entry<UUID, TxContext> ctxEntry : txCtxMap.entrySet()) {
            if (ctxEntry.getValue() instanceof ReadWriteTxContext) {
                ...
                if (txContext.isTxFinishing()) {
                    MutablePartitionEnlistment enlistment = txContext.enlistedGroups.get(groupId);
    ...
    ReadWriteTxContext lockTxForNewUpdates(UUID txId, Map<ReplicationGroupId, MutablePartitionEnlistment> enlistedGroups) {
    ...
    abstract static class TxContext {
        ...
        abstract void finishTx(@Nullable Map<ReplicationGroupId, MutablePartitionEnlistment> enlistedGroups);
    ...
    private static class ReadOnlyTxContext extends TxContext {
        ...
        @Override
        public void finishTx(@Nullable Map<ReplicationGroupId, MutablePartitionEnlistment> enlistedGroups) {
            markedFinished = true;
        }
    ...
    static class ReadWriteTxContext extends TxContext {
        ...
        private volatile Map<ReplicationGroupId, MutablePartitionEnlistment> enlistedGroups;
        private final ClockService clockService;
        ...
        private CompletableFuture<Void> waitReadyToFinish(boolean commit) {
            if (commit) {
                ...
                for (Map.Entry<ReplicationGroupId, MutablePartitionEnlistment> e : enlistedGroups.entrySet()) {
        ...
        @Override
        public void finishTx(Map<ReplicationGroupId, MutablePartitionEnlistment> enlistedGroups) {
