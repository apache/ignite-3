...
public class IgniteComputeImpl implements IgniteCompute {
    ...
    private CompletableFuture<TableView> requiredTable(String tableName) {
    ...
    private static ClusterNode leaderOfTablePartitionByTupleKey(TableView table, Tuple key) {
        return requiredLeaderByPartition(table, table.partition(key));
    }

    private static  <K> ClusterNode leaderOfTablePartitionByMappedKey(TableView table, K key, Mapper<K> keyMapper) {
        return requiredLeaderByPartition(table, table.partition(key, keyMapper));
    }

    private static ClusterNode requiredLeaderByPartition(TableView table, int partitionIndex) {
