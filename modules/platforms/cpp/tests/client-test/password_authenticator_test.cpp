...
struct password_authenticator_test : public password_auth_test_suite {
    /**
     * Tear down.
     */
...
TEST_F(password_authenticator_test, disabled_on_server) {
...
TEST_F(password_authenticator_test, disabled_on_client) {
...
TEST_F(password_authenticator_test, success) {
    set_authentication_enabled(true);
    auto client = ignite_client::start(get_configuration_correct(), std::chrono::seconds(30));
    (void) client.get_cluster_nodes();
}

TEST_F(password_authenticator_test, wrong_username) {
    set_authentication_enabled(true);
    EXPECT_THROW(
        {
            try {
...
TEST_F(password_authenticator_test, wrong_password) {
    set_authentication_enabled(true);
    EXPECT_THROW(
        {
            try {
