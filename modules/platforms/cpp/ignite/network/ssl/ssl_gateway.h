...
public:
     ...
     */
    static ssl_gateway& get_instance();
...
private:
     ...
     * Load SSL library.
     * @param name Name.
     * @param home_dir OpenSSL home directory.
     * @return Module.
     */
    static std::unique_ptr<dynamic_module> load_ssl_library(const std::string& name, const std::string& home_dir);

    /**
     * Load all SSL libraries.
     */
    void load_ssl_libraries();
     ...
     */
    bool try_load_ssl_libraries(const std::string& home_dir);

    /**
     * Load mandatory SSL methods.
     *
     * @throw ignite_error if can not load one of the functions.
     */
    void load_mandatory_methods();
     ...
     */
    void* try_load_ssl_method(const char* name);
     ...
     * @return Method pointer.
     *
     * @throw ignite_error if the method is not present.
     */
    void* load_ssl_method(const char* name);
    ...
    /** libeay32 module. */
    std::unique_ptr<dynamic_module> m_libeay32;

    /** ssleay32 module. */
    std::unique_ptr<dynamic_module> m_ssleay32;

    /** libcrypto module. */
    std::unique_ptr<dynamic_module> m_libcrypto;

    /** libssl module. */
    std::unique_ptr<dynamic_module> m_libssl;

    /** Functions. */
    ssl_functions m_functions{};
};
}
