...
public static class IgniteQueryableExtensions
{
    ...
    public static async Task<TSource?> LastOrDefaultAsync<TSource>(this IQueryable<TSource> queryable)
    {
        IgniteArgumentCheck.NotNull(queryable, nameof(queryable));

        var method = new Func<IQueryable<TSource>, TSource?>(Queryable.LastOrDefault).GetMethodInfo();
        var expression = Expression.Call(null, method, queryable.Expression);
        ...
        Expression<Func<TSource, bool>> predicate)
    {
        ...
        var method = new Func<IQueryable<TSource>, Expression<Func<TSource, bool>>, TSource?>(Queryable.LastOrDefault).GetMethodInfo();
        var expression = Expression.Call(null, method, queryable.Expression, Expression.Quote(predicate));
