            ...
            try
            {
                ...
                while (true)
                {
                    try
                    {
                        ...
                        // TODO: Remove. Always rewrite batch. If this helps, then there is a bug with offsets or schema invalidation.
                        ReWriteBatch(buf, partitionId, schema, items.AsSpan(0, count), writer);
                    ...
                    catch (Exception e) when (e.Message.Contains("Failed to get column value", StringComparison.Ordinal))
                    {
                        ...
                        throw new InvalidOperationException($"Corrupted data: schema={schema.Version}, items=[{itemsStr}], bufOffset={buf.Offset}, bufPost={buf.Position}, buf=[{bufStr}]", e);
                    }
                }
            }
            catch (Exception)
