...
public sealed class JavaServer : IDisposable
{
    ...
    {
        var file = CommonTestUtils.IsWindows ? "cmd.exe" : "/bin/bash";
        ...
        var processStartInfo = new ProcessStartInfo
        {
            ...
            ArgumentList =
            {
                CommonTestUtils.IsWindows ? "/c" : "-c",
                command
            },
            CreateNoWindow = true,
            UseShellExecute = false,
            WorkingDirectory = CommonTestUtils.RepoRootDir,
    ...
    private static string GetGradle()
    {
        ...
        return Path.Combine(CommonTestUtils.RepoRootDir, gradleWrapper);
    }
    ...
    {
        var command = CommonTestUtils.IsWindows
            ? $"for /f \"tokens=5\" %a in ('netstat -aon | find \"{port}\"') do taskkill /f /pid %a"
            : $"kill -9 $(lsof -t -i :{port})";

        var psi = new ProcessStartInfo
        {
            FileName = CommonTestUtils.IsWindows ? "cmd.exe" : "/bin/sh",
            ArgumentList =
            {
                CommonTestUtils.IsWindows ? "/c" : "-c",
                command
