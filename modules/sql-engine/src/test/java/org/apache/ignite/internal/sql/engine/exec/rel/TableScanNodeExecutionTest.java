...
public class TableScanNodeExecutionTest extends AbstractExecutionTest<Object[]> {
    ...
    public void testScanNodeDataPropagation() throws InterruptedException {
        ...
        List<PartitionWithEnlistmentToken> partsWithTokens = IntStream.range(0, TestInternalTableImpl.PART_CNT)
                .mapToObj(p -> new PartitionWithEnlistmentToken(p, -1L))
        ...
        for (int size : sizes) {
            ...
            TableScanNode<Object[]> scanNode = new TableScanNode<>(ctx, rowFactory, scanableTable,
                    partsWithTokens, null, null, null);
            ...
            assertEquals(sizes[i++] * partsWithTokens.size(), cnt);
        }
    }
