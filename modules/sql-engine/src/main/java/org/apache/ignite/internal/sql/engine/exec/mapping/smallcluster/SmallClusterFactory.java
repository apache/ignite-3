...
public class SmallClusterFactory implements ExecutionTargetFactory {
    ...
    @Override
    public ExecutionTarget partitioned(List<NodeWithEnlistmentToken> nodes) {
        long[] partitionNodes = new long[nodes.size()];
        long[] enlistmentConsistencyTokens = new long[nodes.size()];

        int idx = 0;
        for (NodeWithEnlistmentToken e : nodes) {
    ...
    @Override
    public List<NodeWithEnlistmentToken> resolveAssignments(ExecutionTarget target) {
        target = target.finalise();
