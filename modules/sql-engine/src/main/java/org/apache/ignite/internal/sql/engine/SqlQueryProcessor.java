...
public class SqlQueryProcessor implements QueryProcessor {
    ...
    private CompletableFuture<List<NodeWithEnlistmentToken>> primaryReplicas(int tableId) {
        ...
        List<CompletableFuture<NodeWithEnlistmentToken>> result = new ArrayList<>(partitions);
        ...
        for (int partId = 0; partId < partitions; ++partId) {
            ...
            result.add(f.handle((primaryReplica, e) -> {
                if (e != null) {
                    ...
                    return new NodeWithEnlistmentToken(holder, primaryReplica.getStartTime().longValue());
                }
            }));
        }
