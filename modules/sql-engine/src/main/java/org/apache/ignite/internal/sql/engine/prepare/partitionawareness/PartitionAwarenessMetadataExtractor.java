...
public class PartitionAwarenessMetadataExtractor {
    ...
    private static @Nullable PartitionAwarenessMetadata tryConvertPartitionPruningMetadata(
        ...
        // A query can be split into multiple parts where only some of them have PP metadata.
        // Consider the following query:
        //
        // SELECT * FROM t WHERE key IN (?0, 2)
        // 
        // The query can be rewritten into:
        // 
        // SELECT * FROM t WHERE key = 2 (1)
        // UNION ALL
        // SELECT * FROM t WHERE key = ?0 and key != 2 (2) // No PP metadata
        // 
        // We have PP metadata for (1) but (2) can refer to other partitions, 
        // so we cannot guarantee that both queries use the same partition.

        long numRels = numberOfModifyRelsAndDataSources(relationWithSources);
    ...
    private static long numberOfModifyRelsAndDataSources(RelWithSources relationWithSources) {
        Long2ObjectMap<IgniteRel> relations = relationWithSources.relations();
        // When counting the number of source relations ignore safe table functions as they these function 
        // prodce the same result and do not affect / are not affected by data distribution.
