...
/**
 * Tests projection rule {@link ProjectScanMergeRule}.
 *
 * <p>This rule have a deal with only useful columns and for example for tables: T1(f11, f12, f13) and T2(f21, f22, f23)
 * sql execution: SELECT t1.f11, t2.f21 FROM T1 t1 INNER JOIN T2 t2 on t1.f11 = t2.f22" need to eleminate all unused
 * columns and take into account only: f11, f21 and f22 cols.
 */
public class ItProjectScanMergeRuleTest extends BaseSqlIntegrationTest {
    ...
    public void testProjects() {
        ...
        assertQuery("SELECT SUBCAT_ID, NAME FROM products d;")
                .matches(containsProject("PUBLIC", "PRODUCTS", "SUBCAT_ID", "NAME"))
                .returns(11, "noname1")
                .returns(11, "noname2")
                .returns(12, "noname3")
                .returns(13, "noname4")
        ...
        assertQuery("SELECT NAME FROM products d WHERE CAT_ID > 1;")
                .matches(containsProject("PUBLIC", "PRODUCTS", "CATEGORY"))
                .returns("noname2")
                .returns("noname3")
                .returns("noname4")
                .check();
    ...
    public void testNestedProjects() {
        assertQuery("SELECT NAME FROM products WHERE CAT_ID IN (SELECT CAT_ID FROM products WHERE CAT_ID > 1) and ID > 2;")
                .matches(containsProject("PUBLIC", "PRODUCTS", "ID", "CAT_ID", "NAME"))
        ...
        assertQuery("SELECT NAME FROM products WHERE CAT_ID IN (SELECT DISTINCT CAT_ID FROM products WHERE SUBCAT_ID > 11)")
                .matches(containsProject("PUBLIC", "PRODUCTS", "CAT_ID", "NAME"))
                .matches(containsProject("PUBLIC", "PRODUCTS", "ID"))
                .returns("noname3")
                .returns("noname4")
                .check();
