...
public interface BasicAuthProvider extends AuthProvider {
    ...
    /**
     * Creates a new {@link BasicAuthProvider} instance. Acts like a constructor replacement.
     *
     * @param msgFactory Message factory to instantiate builder.
     * @param basicAuthProviderConfig Basic authentication provider config.
     * @return BasicAuthProvider instance.
     */
    static BasicAuthProvider basicAuthProvider(
            CmgMessagesFactory msgFactory,
            BasicAuthProviderConfig basicAuthProviderConfig
    ) {
        return msgFactory.basicAuthProvider()
                .name(basicAuthProviderConfig.name())
                .login(basicAuthProviderConfig.login())
                .password(basicAuthProviderConfig.password())
                .type(basicAuthProviderConfig.type().toString())
                .build();
    }
}
